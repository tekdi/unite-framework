...
3. **Verify MongoDB is active** - Verify MongoDB server is active/running with command ```service mongod status```. This should show status active. If not, start the MongoDB with command ```sudo service mongod start```. **Note** - MongoDB is by default do not started every time the system is restarted / booted. To enable start on boot time, follow this [Link](https://stackoverflow.com/questions/24354318/how-to-get-mongodb-to-start-at-boot-time-in-ubuntu)
